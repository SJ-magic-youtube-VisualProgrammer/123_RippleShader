...
void GUI_GLOBAL::setup(string GuiName, string FileName, float x, float y)
{
	...
	********************/
	...
	Group_FFT.setup("FFT");
		...
		Group_FFT.add(FFT__Afilter_0dB_at_Hz.setup("Afilter:0dB @", 400.0, 80.0, 1000.0));
	gui.add(&Group_FFT);
	...
	Group_ArtSin.setup("ArtSin");
		...
		Group_ArtSin.add(b_TukeyWindow_artSin.setup("b_TukeyWindow_artSin", false));
	...
	Group_WebCam.setup("WebCam");
		Group_WebCam.add(b_DispWebCam.setup("b_DispWebCam", false));
		Group_WebCam.add(b_WebCam_Mirror.setup("b_WebCam_Mirror", true));
		Group_WebCam.add(b_WebCam_Gray.setup("b_WebCam_Gray", true));
	gui.add(&Group_WebCam);
	
	Group_Ripple.setup("Ripple");
		Group_Ripple.add(b_UseRippleShader_toWebCam.setup("b_UseRippleShader_toWebCam", false));
		Group_Ripple.add(Ripple_Gain.setup("RippleGain", 0.255, 0.0, 10.0));
		Group_Ripple.add(Ripple_Space.setup("RippleSpace", 4.1, 1.0, 30.0));
		Group_Ripple.add(Ripple_source.setup("Ripple_source", 6.0, 0.0, 6.0));
	gui.add(&Group_Ripple);
